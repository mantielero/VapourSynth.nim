...
when defined(windows):
  const
    libname* = "VSScript.dll"
elif defined(macosx):
  const
    libname* = "libvapoursynth-script.dylib"
else:
  const
    libname* = "libvapoursynth-script.so"
...
proc vsscript_init*(): cint {.cdecl, importc: "vsscript_init", dynlib: libname.}
##  Free all scripting runtimes

proc vsscript_finalize*(): cint {.cdecl, importc: "vsscript_finalize", dynlib: libname.}
##
...
proc vsscript_evaluateScript*(handle: ptr ptr VSScript; script: cstring;
    ...
    importc: "vsscript_evaluateScript", dynlib: libname.}
...
proc vsscript_evaluateFile*(handle: ptr ptr VSScript; scriptFilename: cstring;
    ...
    importc: "vsscript_evaluateFile", dynlib: libname.}
...
proc vsscript_createScript*(handle: ptr ptr VSScript): cint {.cdecl,
    importc: "vsscript_createScript", dynlib: libname.}
proc vsscript_freeScript*(handle: ptr VSScript) {.cdecl,
    importc: "vsscript_freeScript", dynlib: libname.}
proc vsscript_getError*(handle: ptr VSScript): cstring {.cdecl,
    importc: "vsscript_getError", dynlib: libname.}
...
proc vsscript_getOutput*(handle: ptr VSScript; index: cint): ptr VSNodeRef {.cdecl,
    importc: "vsscript_getOutput", dynlib: libname.}
...
proc vsscript_getVSApi2*(version: cint): ptr VSAPI {.cdecl,
    importc: "vsscript_getVSApi2", dynlib: libname.}
...
proc vsscript_clearEnvironment*(handle: ptr VSScript) {.cdecl,
    importc: "vsscript_clearEnvironment", dynlib: libname.}