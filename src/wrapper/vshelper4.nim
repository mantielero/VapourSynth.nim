# Generated @ 2022-01-07T20:06:09+01:00
# Command line:
#   /home/jose/.nimble/pkgs/nimterop-0.6.13/nimterop/toast -pnkr /usr/include/vapoursynth/VSHelper4.h

# const 'VAPOURSYNTH_API_VERSION' has unsupported value 'VS_MAKE_VERSION(VAPOURSYNTH_API_MAJOR, VAPOURSYNTH_API_MINOR)'
# const 'VSH4_BOOLEAN_TYPE' has unsupported value 'int'
# const 'VS_RESTRICT' has unsupported value 'restrict'
...
{.pragma: impVSHelper4Hdr, header: "/usr/include/vapoursynth/VSHelper4.h".}
...
type
  VSFrame* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSNode* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSCore* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSPlugin* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSPluginFunction* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSFunction* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSMap* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSLogHandle* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSFrameContext* {.importc, impVSHelper4Hdr, incompleteStruct.} = object
  VSPLUGINAPI* {.importc, impVSHelper4Hdr, bycopy.} = object
  ...
  VSAPI* {.importc, impVSHelper4Hdr, bycopy.} = object
  ...
  VSVideoFormat* {.bycopy, impVSHelper4Hdr, importc: "struct VSVideoFormat".} = object
    colorFamily*: cint       ## ```
                             ##   see VSColorFamily
                             ## ```
    sampleType*: cint        ## ```
  ...
  VSAudioFormat* {.bycopy, impVSHelper4Hdr, importc: "struct VSAudioFormat".} = object
    sampleType*: cint
    bitsPerSample*: cint
    bytesPerSample*: cint    ## ```
  ...
  VSCoreInfo* {.bycopy, impVSHelper4Hdr, importc: "struct VSCoreInfo".} = object
  ...
  VSVideoInfo* {.bycopy, impVSHelper4Hdr, importc: "struct VSVideoInfo".} = object
  ...
  VSAudioInfo* {.bycopy, impVSHelper4Hdr, importc: "struct VSAudioInfo".} = object
  ...
  VSGetVapourSynthAPI* {.importc, impVSHelper4Hdr.} = proc (version: cint): ptr VSAPI {.
      cdecl.}
  VSPublicFunction* {.importc, impVSHelper4Hdr.} = proc (`in`: ptr VSMap;
      `out`: ptr VSMap; userData: pointer; core: ptr VSCore; vsapi: ptr VSAPI) {.
      cdecl.}
  VSInitPlugin* {.importc, impVSHelper4Hdr.} = proc (plugin: ptr VSPlugin;
      vspapi: ptr VSPLUGINAPI) {.cdecl.}
  VSFreeFunctionData* {.importc, impVSHelper4Hdr.} = proc (userData: pointer) {.
      cdecl.}
  VSFilterGetFrame* {.importc, impVSHelper4Hdr.} = proc (n: cint;
      ...
      cdecl.}
  VSFilterFree* {.importc, impVSHelper4Hdr.} = proc (instanceData: pointer;
      core: ptr VSCore; vsapi: ptr VSAPI) {.cdecl.}
  VSFrameDoneCallback* {.importc, impVSHelper4Hdr.} = proc (userData: pointer;
      f: ptr VSFrame; n: cint; node: ptr VSNode; errorMsg: cstring) {.cdecl.}
  VSLogHandler* {.importc, impVSHelper4Hdr.} = proc (msgType: cint;
      msg: cstring; userData: pointer) {.cdecl.}
  VSLogHandlerFree* {.importc, impVSHelper4Hdr.} = proc (userData: pointer) {.
      cdecl.}
  VSFilterDependency* {.bycopy, impVSHelper4Hdr,
...
proc getVapourSynthAPI*(version: cint): ptr VSAPI {.importc, cdecl,
    impVSHelper4Hdr.}
proc vsh_isConstantVideoFormat*(vi: ptr VSVideoInfo): cint {.importc, cdecl,
    impVSHelper4Hdr.}
  ## ```
                     ##   convenience function for checking if the format never changes between frames
                     ## ```
proc vsh_isSameVideoFormat*(v1: ptr VSVideoFormat; v2: ptr VSVideoFormat): cint {.
    importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                     ##   convenience function to check if two clips have the same format (unknown/changeable will be considered the same too)
                                     ## ```
proc vsh_isSameVideoPresetFormat*(presetFormat: cuint; v: ptr VSVideoFormat;
                                  core: ptr VSCore; vsapi: ptr VSAPI): cint {.
    importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                     ##   convenience function to check if a clip has the same format as a format id
                                     ## ```
proc vsh_isSameVideoInfo*(v1: ptr VSVideoInfo; v2: ptr VSVideoInfo): cint {.
    importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                     ...
                                     ## ```
proc vsh_isSameAudioFormat*(a1: ptr VSAudioFormat; a2: ptr VSAudioFormat): cint {.
    importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                     ...
                                     ## ```
proc vsh_isSameAudioInfo*(a1: ptr VSAudioInfo; a2: ptr VSAudioInfo): cint {.
    importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                     ##   convenience function to check for if two clips have the same format while also including samplerate (unknown/changeable will be considered the same too)
                                     ## ```
proc vsh_muldivRational*(num: ptr int64; den: ptr int64; mul: int64;
                         `div`: int64) {.importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                                                          ##   multiplies and divides a rational number, such as a frame duration, in place and reduces the result
                                                                          ## ```
proc vsh_reduceRational*(num: ptr int64; den: ptr int64) {.importc, cdecl,
    impVSHelper4Hdr.}
  ## ```
                     ##   reduces a rational number
                     ## ```
proc vsh_addRational*(num: ptr int64; den: ptr int64; addnum: int64;
                      addden: int64) {.importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                                                        ##   add two rational numbers and reduces the result
                                                                        ## ```
proc vsh_int64ToIntS*(i: int64): cint {.importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                                                         ##   converts an int64 to int with saturation, useful to silence warnings when reading int properties among other things
                                                                         ## ```
proc vsh_doubleToFloatS*(d: cdouble): cfloat {.importc, cdecl, impVSHelper4Hdr.}
  ## ```
                                                                                ##   converts a double to float with saturation, useful to silence warnings when reading float properties among other things
                                                                                ## ```
proc vsh_bitblt*(dstp: pointer; dst_stride: ByteAddress; srcp: pointer;
                 src_stride: ByteAddress; row_size: uint; height: uint) {.
    importc, cdecl, impVSHelper4Hdr.}
proc vsh_areValidDimensions*(fi: ptr VSVideoFormat; width: cint; height: cint): cint {.
    importc, cdecl, impVSHelper4Hdr.}
  ## ```
