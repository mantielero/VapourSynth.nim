# Generated @ 2022-01-07T20:07:38+01:00
# Command line:
#   /home/jose/.nimble/pkgs/nimterop-0.6.13/nimterop/toast -pnkr /usr/include/vapoursynth/VSScript4.h

# const 'VAPOURSYNTH_API_VERSION' has unsupported value 'VS_MAKE_VERSION(VAPOURSYNTH_API_MAJOR, VAPOURSYNTH_API_MINOR)'
# const 'VSSCRIPT_API_VERSION' has unsupported value 'VS_MAKE_VERSION(VSSCRIPT_API_MAJOR, VSSCRIPT_API_MINOR)'
...
{.pragma: impVSScript4Hdr, header: "/usr/include/vapoursynth/VSScript4.h".}
...
type
  VSFrame* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSNode* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSCore* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSPlugin* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSPluginFunction* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSFunction* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSMap* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSLogHandle* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSFrameContext* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSPLUGINAPI* {.importc, impVSScript4Hdr, bycopy.} = object
  ...
  VSAPI* {.importc, impVSScript4Hdr, bycopy.} = object
  ...
  VSVideoFormat* {.bycopy, impVSScript4Hdr, importc: "struct VSVideoFormat".} = object
    colorFamily*: cint       ## ```
                             ##   see VSColorFamily
                             ## ```
    sampleType*: cint        ## ```
  ...
  VSAudioFormat* {.bycopy, impVSScript4Hdr, importc: "struct VSAudioFormat".} = object
    sampleType*: cint
    bitsPerSample*: cint
    bytesPerSample*: cint    ## ```
  ...
  VSCoreInfo* {.bycopy, impVSScript4Hdr, importc: "struct VSCoreInfo".} = object
  ...
  VSVideoInfo* {.bycopy, impVSScript4Hdr, importc: "struct VSVideoInfo".} = object
  ...
  VSAudioInfo* {.bycopy, impVSScript4Hdr, importc: "struct VSAudioInfo".} = object
  ...
  VSGetVapourSynthAPI* {.importc, impVSScript4Hdr.} = proc (version: cint): ptr VSAPI {.
      cdecl.}
  VSPublicFunction* {.importc, impVSScript4Hdr.} = proc (`in`: ptr VSMap;
      `out`: ptr VSMap; userData: pointer; core: ptr VSCore; vsapi: ptr VSAPI) {.
      cdecl.}
  VSInitPlugin* {.importc, impVSScript4Hdr.} = proc (plugin: ptr VSPlugin;
      vspapi: ptr VSPLUGINAPI) {.cdecl.}
  VSFreeFunctionData* {.importc, impVSScript4Hdr.} = proc (userData: pointer) {.
      cdecl.}
  VSFilterGetFrame* {.importc, impVSScript4Hdr.} = proc (n: cint;
      ...
      cdecl.}
  VSFilterFree* {.importc, impVSScript4Hdr.} = proc (instanceData: pointer;
      core: ptr VSCore; vsapi: ptr VSAPI) {.cdecl.}
  VSFrameDoneCallback* {.importc, impVSScript4Hdr.} = proc (userData: pointer;
      f: ptr VSFrame; n: cint; node: ptr VSNode; errorMsg: cstring) {.cdecl.}
  VSLogHandler* {.importc, impVSScript4Hdr.} = proc (msgType: cint;
      msg: cstring; userData: pointer) {.cdecl.}
  VSLogHandlerFree* {.importc, impVSScript4Hdr.} = proc (userData: pointer) {.
      cdecl.}
  VSFilterDependency* {.bycopy, impVSScript4Hdr,
  ...
  VSScript* {.importc, impVSScript4Hdr, incompleteStruct.} = object
  VSSCRIPTAPI* {.importc, impVSScript4Hdr, bycopy.} = object
    getAPIVersion*: proc (): cint {.cdecl.} ## ```
                                            ##   Returns the highest supported VSSCRIPT_API_VERSION
                                            ## ```
    getVSAPI*: proc (version: cint): ptr VSAPI {.cdecl.} ## ```
                                                         ...
                                                         ## ```
    createScript*: proc (core: ptr VSCore): ptr VSScript {.cdecl.} ## ```
                                                                   ...
                                                                   ## ```
    getCore*: proc (handle: ptr VSScript): ptr VSCore {.cdecl.} ## ```
                                                                ##   The core is valid as long as the environment exists, return NULL on error
                                                                ## ```
    evaluateBuffer*: proc (handle: ptr VSScript; buffer: cstring;
                           scriptFilename: cstring): cint {.cdecl.} ## ```
                                                                    ...
                                                                    ## ```
    evaluateFile*: proc (handle: ptr VSScript; scriptFilename: cstring): cint {.
        cdecl.} ## ```
                ...
                ## ```
    getError*: proc (handle: ptr VSScript): cstring {.cdecl.} ## ```
                                                              ##   Returns NULL on success, otherwise an error message
                                                              ## ```
    getExitCode*: proc (handle: ptr VSScript): cint {.cdecl.} ## ```
                                                              ##   Returns the script's reported exit code
                                                              ## ```
    getVariable*: proc (handle: ptr VSScript; name: cstring; dst: ptr VSMap): cint {.
        cdecl.} ## ```
                ...
                ## ```
    setVariables*: proc (handle: ptr VSScript; vars: ptr VSMap): cint {.cdecl.} ## ```
                                                                                ##   Sets all keys in the provided VSMap as variables in the script. Returns 0 on success.
                                                                                ## ```
    getOutputNode*: proc (handle: ptr VSScript; index: cint): ptr VSNode {.cdecl.} ## ```
                                                                                   ...
                                                                                   ## ```
    getOutputAlphaNode*: proc (handle: ptr VSScript; index: cint): ptr VSNode {.
        cdecl.}
    getAltOutputMode*: proc (handle: ptr VSScript; index: cint): cint {.cdecl.}
    freeScript*: proc (handle: ptr VSScript) {.cdecl.}
    evalSetWorkingDir*: proc (handle: ptr VSScript; setCWD: cint) {.cdecl.} ## ```
...
proc getVapourSynthAPI*(version: cint): ptr VSAPI {.importc, cdecl,
    impVSScript4Hdr.}
proc getVSScriptAPI*(version: cint): ptr VSSCRIPTAPI {.importc, cdecl,
    impVSScript4Hdr.}
{.pop.}
